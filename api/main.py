...
class PatientData(BaseModel):
    temperature: float = Field(..., ge=0, le=300, description="Body temperature in Fahrenheit")
    ...
    heart_rate: int = Field(..., ge=0, le=220, description="Heart rate in beats per minute")
    ...
    chronic_conditions: str = Field(..., description="Presence of chronic conditions (Yes/No/None)")
    allergies: str = Field(..., description="Presence of allergies (Yes/No)")
    smoking_history: str = Field(..., description="Smoking history (Non-smoker/Former/Current)")
    alcohol_consumption: str = Field(..., description="Alcohol consumption (None/Occasional/Regular)")
    physical_activity: str = Field(..., description="Physical activity level (Low/Moderate/High)")
    diet_type: str = Field(..., description="Diet type (Balanced/Vegetarian/Vegan/Other)")
    blood_pressure: str = Field(..., description="Blood pressure category (Normal/High/Low)")
    previous_medication: str = Field(..., description="Previous medication usage (Yes/No/None)")
    recommended_medication: str = Field(..., description="Recommended medication (Paracetamol/Ibuprofen/None)")
...
async def predict(patient_data: PatientData):
    ...
    try:
        logger.info(f"Received prediction request for patient: {{patient_data.age}} years")
        ...
        logger.info(f"Prediction completed: {{risk_level}} risk (probability: {{probability:.3f}})")
...
async def get_patient_risk(
    ...
    print(f"🔍 Debug: Patient risk endpoint called for patient_id={patient_id}, include_details={include_details}")
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Patient risk assessment failed: {str(e)}")
