...
def load_processed_data(
    ...
    target_col: str = "Fever_Severity_code",
):
    ...
    if target_col not in train.columns and "Fever_Severity" in train.columns:
        ...
        print(
            f"✅ Converted 'Fever_Severity' → '{target_col}' with classes: {list(le.classes_)}"
        )
    ...
    if target_col not in train.columns or target_col not in test.columns:
        raise KeyError(
            f"Target column '{target_col}' not found in train/test CSVs. Available columns: {list(train.columns)}"
        )
    ...
    print(
        f"✅ Loaded data: {X_train.shape[0]} train samples, {X_test.shape[0]} test samples"
    )
    print(
        f"✅ Target distribution - Train: {y_train.value_counts().to_dict()}, Test: {y_test.value_counts().to_dict()}"
    )
...
def build_default_preprocessor(
    numerical_cols, categorical_cols, preprocessing_params=None
):
    preprocessing_params = preprocessing_params or {}
    num_imputer = SimpleImputer(
        strategy=preprocessing_params.get("numerical", {}).get("imputer", "median")
    )
    scaler = StandardScaler()
    num_pipe = Pipeline([("imputer", num_imputer), ("scaler", scaler)])

    cat_imputer = SimpleImputer(
        strategy=preprocessing_params.get("categorical", {}).get(
            "imputer", "most_frequent"
        )
    )

    handle_unknown = preprocessing_params.get("categorical", {}).get(
...
def train_all_models():
    ...
    # Set up MLflow
    tracking_uri = params.get("training", {}).get("tracking_uri") or os.environ.get(
        "MLFLOW_TRACKING_URI"
    )
    if tracking_uri:
    ...
    if HAVE_PROJECT_PREPROCESSOR:
        try:
            preprocessor = project_get_preprocessor(
                numerical_cols=numerical_cols, categorical_cols=categorical_cols
            )
            print("✅ Using project's get_data_preprocessor()")
        except Exception as e:
            print(
                "⚠️ project_get_preprocessor failed, falling back to default preprocessor:",
                e,
            )
            preprocessor = build_default_preprocessor(
                numerical_cols, categorical_cols, preproc_params
            )
    else:
        preprocessor = build_default_preprocessor(
            numerical_cols, categorical_cols, preproc_params
    ...
    MODELS_TO_TRAIN = {
        ...
        "logistic_regression": (
            LogisticRegression,
            model_config.get("logistic_regression", {}),
        ),
        ...
        "decision_tree": (
            ...
            model_config.get("decision_tree", {}),
        ),
    }
