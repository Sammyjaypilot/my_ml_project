...
class ModelTrainer:
    ...
    def __init__(self, params_path: Path = None):
        self.params_path = params_path or Path(
    ...
    def train_and_evaluate(
        self, X_train, X_test, y_train, y_test, preprocessor, use_mlflow=True
    ):
        """
        ...
        for model_name, pipeline in pipelines.items():
            ...
            if mlflow_enabled:
                try:
                    experiment_name = self.params.get("training", {}).get(
                        "experiment_name", "FeverSeverity_Comparison"
                    )
                    ...
                    with mlflow.start_run(run_name=f"{model_name}_baseline"):
                        ...
                        mlflow.log_param(
                            "random_state",
                            self.params.get("model", {}).get("random_state", 42),
                        )
                ...
                pipeline.fit(X_train, y_train)
                metrics = self.evaluate_model(pipeline, X_test, y_test, model_name)
                self.results[model_name] = {"pipeline": pipeline, "metrics": metrics}
    ...
    def select_best_model(self, metric="roc_auc"):
        ...
        self.best_model_name = max(
            valid_models, key=lambda x: valid_models[x]["metrics"].get(metric, -1)
        )
        self.best_model = self.results[self.best_model_name]["pipeline"]

        print(f"\n🎉 Best Model: {self.best_model_name}")
        print(
            f"🏆 Best {metric.upper()}: {self.results[self.best_model_name]['metrics'].get(metric, -1):.4f}"
        )
