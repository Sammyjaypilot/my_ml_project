...
def run_experiments():
    ...
    try:
        ...
        print(
            f"✅ Loaded processed data: {X_train.shape[0]} train, {X_test.shape[0]} test"
        )
        ...
        print(
            f"✅ Loaded and split raw data: {X_train.shape[0]} train, {X_test.shape[0]} test"
        )
    ...
    for model_type in model_types:
        ...
        try:
            ...
            filtered_params = filter_valid_params(
                MODEL_CLASS_MAP[model_type], model_params
            )
            ...
            tp.save_training_report(
                test_report, filepath=f"reports/{model_type}_metrics.json"
            )

            # Register model
            tp.register_model(
                model_name="FeverSeverityModel", transition_to_stage="Staging"
            )
